classdef fit_biClarkKyClamped < clarkfitGUI
   properties
       
   end
   
   methods
       function hGUI=fit_biClarkKyClamped(params,fign)
           % INITIALIZATION
           if nargin == 0
               params = struct;
               fign=10;
           ...
           hGUI.modelFx = @cModelBi_Ky_clamped;     
           hGUI.n = 08;
           hGUI.names = {...
               '<html>&tau;Z</html>',...
               '<html>nz</html>',...
...
%            params=checkStructField(params,'ini',[0178,0100,0790,0851,0950]); % restricting gamma c[0,1] and nz > 1; seems like these are really the best fit params but model is definitely trying to get average good behavior
           ...
           params=checkStructField(params,'ini',[0166,0100,0448,0194,0360,0287,10.7,0246]); % restricting gamma c[0,1] and nz > 1; seems like these are really the best fit params but model is definitely trying to get average good behavior
...
%            params=checkStructField(params,'lower',[0 0 100 0 0]);
           params=checkStructField(params,'upper',[Inf Inf Inf Inf Inf Inf Inf Inf]);
