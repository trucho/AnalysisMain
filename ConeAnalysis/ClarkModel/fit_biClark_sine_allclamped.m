classdef fit_biClark_sine_allclamped < ephysGUI
    properties
        
        coeffs = [44.1]; % coeffs for best fit to stj
        guesscoeffs = [44.1]; % manual fit
...
%                 c7: 150 pA
    ...
    methods
        ...
        function hGUI=fit_biClark_sine_allclamped(fign)
            % INITIALIZATION
            if nargin == 0
                fign=10;
            end
...
%             hGUI.ylim(hGUI.gObj.p_exResp01,[-70 40])
        ...
        function runLSQ(hGUI,~,~)
           ...
           LSQ.ydata=hGUI.coneExData(i2use,:)-prctile(hGUI.coneExData(i2use,:),3);
...
%            keyboard
    ...
    methods (Static=true)
        function [ios]=cmodel(coeffs,time,stim,dt, verboseflag)
           ios = cModelBi_allclamped(coeffs,time,stim,dt,verboseflag);
           ios = ios * 150/136;
       end
    end
