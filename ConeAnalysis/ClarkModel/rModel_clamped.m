function [ios]=rModel_clamped(coef,time,stim,~,varargin)
...
opsin_gain=coef(2); %so stimulus can be in R*/sec (this is rate of increase in opsin activity per R*/sec)

% rescaling to get parameters into similar ranges
%Fixed parameters
hillaffinity = 492/1000;		% affinity for Ca2+ (~0.5*dark Calcium)
sigma = 228/10;			% rhodopsin activity decay rate constant (1/sec) ()
phi = sigma;              % phosphodiesterase activity decay rate constant (1/sec) ()
eta = 8412;				% phosphodiesterase activation rate constant (1/sec) ()
% eta/phi~duration of response (50ms) (main determinant of response kinetics)
