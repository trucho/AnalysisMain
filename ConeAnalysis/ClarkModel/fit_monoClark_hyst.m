classdef fit_monoClark_hyst < hystfitGUI
   properties
       
   end
   
   methods
       function hGUI=fit_monoClark_hyst(params,fign)
           % INITIALIZATION
           if nargin == 0
               params = struct;
               fign=10;
...
%            params=checkStructField(params,'ini',[49.4,0124,0407,0.505,0341,41.7,0140,0223]);
           ...
           % fit directly to hyst data with biClark -> removing extra params
%            params=checkStructField(params,'ini',[0115,0209,0276,14.1,37.9,0148,0907,0861]);
           % then ran lsq starting from there
           params=checkStructField(params,'ini',[0130,48.8,0204,0120,0469,0141,1200,1510]);
